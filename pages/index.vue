        ...
        <aside class="w-1/5 border-r border-gray-200">
            <div class="flex flex-col sticky top-[80px]">
                ...
                <div class="categories p-2 flex flex-col gap-2">
                    <button v-for="item in storeCategories" :class="{ 'selected': selectedCategory.includes(item.category)  }" @click="filterCategory(item.category)">
...
<script setup>
    ...
    const storeCategories = computed(() => {
        const categoryMap = storeProducts.value.reduce((acc, product) => {
    ...
    const visibleProducts = computed(() => selectedCategory.value.length === 0 ? storeProducts.value : storeProducts.value.filter(product => selectedCategory.value.includes(product.category)));
...
<style scoped>
    ...
    .categories button {
        @apply px-4 flex justify-between items-center h-[40px] capitalize bg-gray-100 hover:bg-gray-50;
    }
